---
id: source-business-logic-042
type: source-card
course: business-logic
lecture_id: "042"
teacher: 高玮
title: 第一性原理1：埃隆·马斯克的成功秘笈
source_format: transcript
source_type: course-transcript
source_path: "D:\\Space\\notes\\HomeSpace\\Finance\\Gao\\Business Logic\\LectureText\\042、业务全链路思维方式.txt"
status: inventoried
rights: private-reference
created: 2026-05-14
updated: 2026-05-17
---

# 来源卡片：第一性原理1：埃隆·马斯克的成功秘笈

## 来源说明

本来源为高玮商业逻辑课文字稿。

核验说明：当前 `LectureText` 目录中，042 文件名显示为 `042、业务全链路思维方式.txt`，但正文开场明确标注为第四十二回，并实际讲授“第一性原理1：埃隆·马斯克的成功秘笈”。因此本卡片按编号锁定当前 042 文件，标题按正文主题记录。

## 文件信息

- 本地路径：`D:\Space\notes\HomeSpace\Finance\Gao\Business Logic\LectureText\042、业务全链路思维方式.txt`
- 格式：txt
- 课程：商业逻辑
- 讲次：042

## 已处理产物

- 讲次页面：`courses/business-logic/lectures/042-first-principles-1.md`

## 事实摘录

尚未摘录。

## 推断和待验证

- 当前 042 文件名与正文主题存在错位，后续提炼时需要以正文主题为准。
- 需要确认本讲与第 043 讲的关系。

## 关联页面

- `wiki/frameworks/first-principles.md`
