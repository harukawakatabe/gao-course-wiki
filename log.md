...
- 确定第一批资料来源为商业逻辑课文字稿目录，后续已统一迁移到 `D:\Project\Lecture\gao-course-raw\LectureText\Transcript`。
...
- 读取来源卡片 `sources/cards/source-business-logic-001.md`，完成讲次 001 的首轮提炼。
...
- 复核讲次 018 的原始文字稿后，确认此前提炼主题有误，重新按 ChatGPT 与 AI 能力边界主题提炼。
- 删除错误的旧讲次页与旧 wiki 页，改写讲次页面 `courses/business-logic/lectures/018-ai-chatgpt-human-work.md`。
...
## 2026-05-18 | invalid-lecture-outputs-removed | 删除错误回数绑定的派生产物

- 根据 `sources/raw-paths.md` 与当前原始目录复核，确认 032、040、042 的既有派生产物已不再对应正确回数。
- 删除相关讲次页面、来源卡片与 wiki 页面，保留原始来源盘点作为事实层。
- 更新课程索引、wiki 索引与 README，等待前置讲次完成后按正确回数重新提炼。
